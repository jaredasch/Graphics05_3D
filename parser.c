...
void parse_file(char * filename, struct matrix * transform, struct matrix * edges, screen s) {
    ...
    while (fgets(line, 255, f) != NULL) {
        ...
        if (strcmp(line, "line") == 0) {
            ...
            fgets(line, 1024, f);
            args = parse_args(line);
            add_box(edges, atof(args[0]), atof(args[1]), atof(args[2]), atof(args[3]), atof(args[4]), atof(args[5]));
        } else if (strcmp(line, "sphere") == 0){
            fgets(line, 1024, f);
            args = parse_args(line);
            add_sphere(edges, atof(args[0]), atof(args[1]), atof(args[2]), atof(args[3]), 20);
        } else if (strcmp(line, "torus") == 0){
            fgets(line, 1024, f);
            args = parse_args(line);
            add_torus(edges, atof(args[0]), atof(args[1]), atof(args[2]), atof(args[3]), atof(args[4]), 20);
