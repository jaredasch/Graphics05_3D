...
void add_sphere( struct matrix * edges, double cx, double cy, double cz, double r, int step ) {
    ...
    for(int point = 0; point < points->lastcol; point++){
        add_edge(edges, points->m[0][point], points->m[1][point], points->m[2][point], points->m[0][point], points->m[1][point], points->m[2][point]);
    }
    return;
}
...
struct matrix * generate_sphere(double cx, double cy, double cz, double r, int step ) {
    ...
    for(int i = 0; i < step; i++){
        ...
        for(int j = 0; j < step; j++){
            theta = j * M_PI / step;
            add_point(points, r * cos(theta) + cx, r * sin(theta) * cos(phi) + cy, r * sin(theta) * sin(phi) + cz);
        }
    }
    return points;
}
...
struct matrix * generate_torus( double cx, double cy, double cz, double r1, double r2, int step ) {
    ...
    for(int i = 0; i < step; i++){
        ...
        for(int j = 0; j < step; j++){
            theta = j * 2 * M_PI / step;
            add_point(points, cos(phi) * (r1 * cos(theta) + r2) + cx, r1 * sin(theta) + cy, sin(phi) * (r1 * cos(theta) + r2) + cz);
        }
    }
